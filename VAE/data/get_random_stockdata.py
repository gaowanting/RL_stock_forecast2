...
def get_random_data():
    ...
    while True:
        # 在上市公司中随机抽取股票一支股票代码
        tic_on_market = pro.stock_basic(exchange='', list_status='L',fields='ts_code, name, area,industry,list_date')
        tic = random.choice(tic_on_market["ts_code"])
        ...
        data_tmp = ts.pro_bar(ts_code=tic, adj='qfq',freq='5',
        ...
        if (data_tmp is not None) and (len(data_tmp)>2000):
            ...
            print(tic_on_market[tic_on_market["ts_code"] == tic])
            break
    return data_tmp
